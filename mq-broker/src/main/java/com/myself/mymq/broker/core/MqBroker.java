package com.myself.mymq.broker.core;
...
import com.myself.mymq.broker.api.IMqBroker;
import com.myself.mymq.broker.constant.BrokerConst;
import com.myself.mymq.broker.constant.BrokerRespCode;
import com.myself.mymq.broker.dto.consumer.ConsumerSubscribeBo;
import com.myself.mymq.broker.handler.MqBrokerHandler;
import com.myself.mymq.broker.support.api.LocalBrokerConsumerService;
import com.myself.mymq.broker.support.api.LocalBrokerProducerService;
import com.myself.mymq.broker.support.persist.IMqBrokerPersist;
import com.myself.mymq.broker.support.persist.LocalMqBrokerPersist;
import com.myself.mymq.broker.support.push.BrokerPushService;
import com.myself.mymq.broker.support.push.IBrokerPushService;
import com.myself.mymq.broker.support.valid.BrokerRegisterValidService;
...
public class MqBroker extends Thread implements IMqBroker {
    ...
    private ChannelHandler initChannelHandler(){
        ...
        handler.invokeService(invokeService)//调用管理类
                .respTimeoutMills(respTimeoutMills)//响应超时时间
                .registerConsumerService(registerConsumerService)//消费者管理
                .registerProducerService(registerProducerService)//生产者管理
                .mqBrokerPersist(mqBrokerPersist)//持久化类
                .brokerPushService(brokerPushService)//推送服务
                .respTimeoutMills(respTimeoutMills)//响应超时时间
                .pushMaxAttempt(pushMaxAttempt)//最大尝试次数
                .brokerRegisterValidService(brokerRegisterValidService);//验证服务类
    ...
    public void run(){
        //启动服务端
        log.info("MQ中间人开始启动服务端 port：{}",port);
        ...
        try{
            ...
            log.info("MQ中间人启动完成，监听【"+port+"】端口");

            channelFuture.channel().closeFuture().syncUninterruptibly();
            log.info("中间人关闭完成");
        }catch (Exception e){
            log.error("MQ中间人启动异常",e);
