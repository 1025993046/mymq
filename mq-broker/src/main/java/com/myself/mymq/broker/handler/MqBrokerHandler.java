...
public class MqBrokerHandler extends SimpleChannelInboundHandler {
    ...
    private MqCommonResp handleProducerSendMsg(String channelId,String json){
        ...
        //消息体MQMessage、地址信息rpcAddress、消息状态messageStatus
        persistPut.setMqMessage(mqMessage);
        persistPut.setMessageStatus(MessageStatusConst.WAIT_CONSUMER);//这个是待消费状态
        //构建rpc信息
        //生产者管理registerProducerService
        final ServiceEntry serviceEntry=registerProducerService.getServiceEntry(channelId);
        //分组名称groupName、地址address、端口port、权重weight
    ...
    private void asyncHandleMessage(MqMessagePersistPut put){
        ...
        //消费者管理registerConsumerService
        //ChannelGroupNameDto包含了分组名称consumerGroupName，通道channel
        ...
        BrokerPushContext brokerPushContext=BrokerPushContext.newInstance()
                ...
                .invokeService(invokeService)//调用管理类
                .respTimeoutMills(respTimeoutMills)//获取响应超时时间
                .pushMaxAttempt(pushMaxAttempt);//推送最大尝试次数
